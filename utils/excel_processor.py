...
def classify_cases(df_salla: pd.DataFrame, df_abc: pd.DataFrame) -> pd.DataFrame:
    ...
    text_columns = [
        ...
        "salla_branch_number", "abc_branch_number", "profile_type",
        "receipt_classification", "all_abc_pharmacies", "other_branch_details"
    ]
    ...
    result["item_key"] = result.apply(lambda row: f"{row['pharmacy_name']}||{row['order_number']}||{row['sku']}||{row['case_type']}", axis=1)
