        ...
        if st.button("🔄 معالجة البيانات", use_container_width=True, type="primary"):
            with st.spinner("جاري معالجة البيانات..."):
                try:
                    ...
                    for _, row in df_orders.iterrows():
                        ...
                        try:
                            ...
                            for item in json_data:
                                ...
                                # 💡 التعديل هنا: إضافة البوكس الأساسي دائماً سواء كان له فروع أم لا
                                if combined_sku:
                                    final_rows.append({
                                        'رقم الطلب': order_id,
                                ...
                                # 💡 إدراج المنتجات الفرعية بشكل مستقل ومفصل
                                    ...
                                    for sub_idx, sub in enumerate(sub_items_list):
                                        ...
                                        if sub_combined_sku and actual_sub_qty > 0:
                                            final_rows.append({
                                                ...
                                                'الإجمالي': sub_total,
                                                'النوع': 'فرعي',
                                                'حالة المنتج': sub_payment_status,
                                                'الخصم': order_info.get('الخصم', 0),
                                                'تكلفة الشحن': order_info.get('تكلفة الشحن', 0),
                                                'طريقة الدفع': order_info.get('طريقة الدفع', 'غير محدد'),
                                                'الضريبة': order_info.get('الضريبة', 0),
                    ...
                    # 💡 التعديل الجذري الأهم: إضافة "النوع" و "SKU مجمع" لعملية التجميع (Group By)
                    # هذا سيمنع دمج سطر البوكس الأساسي مع سطر المنتجات الفرعية نهائياً
                    group_cols = [
                        'رقم الطلب', 'SKU فردي', 'SKU مجمع (للمراجعة)', 'النوع', 'حالة المنتج', 'الخصم', 'تكلفة الشحن', 'طريقة الدفع', 
                    ...
                    st.success(f"✅ تمت المعالجة بنجاح وتم استخراج مكونات المنتجات بدقة!")
